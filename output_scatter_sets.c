...
void f_scatter_print(scatter* scat, FILE* output) {
	vec_print(scat->loc, output);
	fprintf(output, ", %f", scat->deposit);
}

void f_scat_list_print(uint hist_num, uint gamma_num, llist* scatters, FILE* output, int first, int second) {
	uint scat_num = 0;
	while (scatters != NULL) {
		fprintf(output, "%i, %i, %i, ", hist_num, gamma_num, scat_num);
		f_scatter_print(scatters->data, output);
		if (scat_num == first) {
			fprintf(output, ", 1");
		} else {
			fprintf(output, ", 0");
			...
			for (int k = 0; k < len_hist1; k++) {
				...
				if (j != k) {
					...
					if (fabs(hypoth - ELECTRON_MASS) < fabs(best_find - ELECTRON_MASS)) {
						...
						predicted_vs_real[2 * (run_num - 1)] = j;
						predicted_vs_real[2 * (run_num - 1) + 1] = k;
					}
				}
			}
		}
...
llist** build_scat_lists(llist* detector_history) {
	...
	return_lists[3] = (llist*)second_id;
	return return_lists;
}
	...
	while ((history != NULL) && (hist_num < max_hist)) {
		...
		if (((event*)in_det_hist->data)->number == ((event*)history->data)->number) {
			...
			if (endpoints == NULL) {
				...
				fmap(scatter_lists[0], delete_scatter);
				fmap(scatter_lists[1], delete_scatter);
				delete_list(scatter_lists[0]);
				delete_list(scatter_lists[1]);
				free(scatter_lists);
