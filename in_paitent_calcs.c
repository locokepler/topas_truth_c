...
scatter* new_scatter(vec3* vector, double deposited) {
 ...
 */
double expected_energy(scatter* a, scatter* b, scatter* c) {
...
int test_expected_energy() {
	...
	scatter* scatter_a = new_scatter(point_a, deposit_a);
	scatter* scatter_b = new_scatter(point_b, deposit_b);
	scatter* scatter_c = new_scatter(point_c, 10.);
	// run the function
	double result = expected_energy(scatter_a, scatter_b, scatter_c);
 ...
 * Method: Calls expected_energy on every possible orientation of the list of
...
scatter* scattering_iterator(llist* history, double energy_percent) {
		...
		for (int second = 0; second < list_len; second++)
		{
			for (int third = 0; third < list_len; third++)
			{
				if ((first != second) && (first != third) && (second != third)) {
					hypoth = expected_energy(locations[first],
											locations[second],
											locations[third]);
					...
					if (fabs(hypoth - ELECTRON_MASS) < fabs(best_find - ELECTRON_MASS)) {
						...
						hypoth_change = fabs(hypoth - best_find);
						best_find = hypoth;
	...
	if (first_scat_hypot == 0) {
		first_scat_hypot = hypoth_change;
	} else {
		second_scat_hypot = hypoth_change;
	}
...
llist* build_scatters(llist* detector_history, int id) {
	...
	while (detector_history != NULL) {
		...
		if (((event*)detector_history->data)->particle == 11) {
			...
			// electron
			if (!electron_checked) {
				// if we haven't looked at this one yet, so which gamma is it from?
				...
				if (gamma_id == id) {
					...
					scatter_list = add_to_bottom(scatter_list, new_scatter(scatter_loc, ((event*)detector_history->data)->energy));
				}
...
scatter** find_endpoints(llist* detector_history, double energy_percent) {
	...
	scatter* first_endpoint = new_scatter(vec_copy(endpoint->loc), endpoint->deposit);
	...
	scatter* second_endpoint = new_scatter(vec_copy(endpoint->loc), endpoint->deposit);
	fmap(scat_list, delete_scatter);
	delete_list(scat_list);
